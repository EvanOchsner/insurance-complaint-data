"""Pull the TDI 'Insurance complaints: One record / complaint' dataset from
data.texas.gov and write a Polars Parquet plus a manifest.

Source: https://data.texas.gov/dataset/Insurance-complaints-One-record-complaint/jjc8-mxkg
Resource: https://data.texas.gov/resource/jjc8-mxkg.json

Anonymous Socrata access. ~281k rows; ~6 paginated GETs at 50k page size.
"""
...
RESOURCE_URL = f"https://data.texas.gov/resource/{DATASET_ID}.json"
...
INTERIM_DIR = PROJECT_ROOT / "tx_tdi" / "interim"
...
def main() -> int:
    ...
    # Build a Polars frame. All Socrata resource columns arrive as strings or
    # ISO datetimes-as-strings; we keep them as strings here and parse in
    # 02_aggregate.py to keep the interim file faithful. Forcing every column
    # to pl.String avoids schema-inference races (some rows omit optional
    # fields entirely; Polars otherwise picks a non-string type from the
    # first observed value and chokes when a later row needs to coerce).
    column_names = [c[0] for c in meta["columns"]]
    schema = {name: pl.String for name in column_names}
    # Normalize: ensure every row has every column key.
